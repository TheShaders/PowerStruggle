...
typedef struct BVHNode_t {
    uint16_t triCount; // 0 if this is not a leaf node, otherwise the number of triangles in this node
    uint16_t firstTriIndex; // Index of the first triangle for this node
    int16_t missNodeIndex; // Index of the node to move to if this AABB test is a miss
    uint16_t reserved;
    AABB aabb;
} BVHNode;
...
typedef struct ColliderParams_t {
    ...
    ColTri *floor; // The floor this object is on (nullptr if it's in the air)
