...
float verticalRayVsBvh(Vec3 rayStart, float length, BVHTree *bvh, float tmin, float tmax, ColTri **hitOut, SurfaceType *floorSurface)
{
    ...
    ColTri *hitTri = nullptr;
    SurfaceType hitSurfaceType = SURFACE_NONE;
    BVHNode *nodes = segmentedToVirtual(bvh->nodes);
    BVHNode *curNode = nodes;
    ColTri *tris = segmentedToVirtual(bvh->tris);
    SurfaceType *surfaceTypes = segmentedToVirtual(bvh->surfaceTypes);
...
{
    BVHTree **curTree = static_cast<BVHTree**>(componentArrays[COMPONENT_INDEX(Collision, ARCHETYPE_COLLISION)]);
...
{
    BVHTree **curTree = static_cast<BVHTree**>(componentArrays[COMPONENT_INDEX(Collision, ARCHETYPE_POS_COLLISION)]);
    Vec3 *curPos = static_cast<Vec3*>(componentArrays[COMPONENT_INDEX(Position, ARCHETYPE_POS_COLLISION)]);
    RaycastVerticalData *rayData = (RaycastVerticalData *)data;
    ColTri *curTri;
    SurfaceType curSurfaceType;
    Vec3 origin = { rayData->origin[0], rayData->origin[1], rayData->origin[2] };
...
{
    BVHTree **curTree = static_cast<BVHTree**>(componentArrays[COMPONENT_INDEX(Collision, ARCHETYPE_POSROT_COLLISION)]);
    Vec3 *curPos = static_cast<Vec3*>(componentArrays[COMPONENT_INDEX(Position, ARCHETYPE_POSROT_COLLISION)]);
    Vec3s *curRot = static_cast<Vec3s*>(componentArrays[COMPONENT_INDEX(Rotation, ARCHETYPE_POSROT_COLLISION)]);
    RaycastVerticalData *rayData = (RaycastVerticalData *)data;
    ColTri *curTri;
    SurfaceType curSurfaceType;
    Vec3 origin = { rayData->origin[0], rayData->origin[1], rayData->origin[2] };
...
{
    RaycastVerticalData data = {rayOrigin, rayLength, tmin, tmax, nullptr, SURFACE_NONE, FLT_MAX};
...
{
    RaycastData data = {rayOrigin, rayDir, tmin, tmax, nullptr, SURFACE_NONE, FLT_MAX};
